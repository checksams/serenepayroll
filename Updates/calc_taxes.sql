...
BEGIN
  ...
  set @cursor_tax = cursor FAST_FORWARD for
	...
	from shr_taxes  t left join shr_tax_rates r
...
set @v_tax_amt = 0
	...
	where tx_code = @v_tx_code --and v_dt between tx_wef and  tx_wet
					...
					/*if @v_tx_code = 2
					begin
						select @v_error = 'Unable to compute tax amount to count='+ CONVERT(nvarchar,@v_cnt)
						+ '@c_txr_rate=' + CONVERT(nvarchar,@c_txr_rate)
						+ '@c_txr_range_from=' + CONVERT(nvarchar,@c_txr_range_from)
						+ '@c_txr_range_to=' + CONVERT(nvarchar,@c_txr_range_to)
